...
function isEarlyShellRevealEnabled() {
  return BT_EARLY_SHELL_REVEAL === true;
}
...
function isEarlyShellRevealReady() {
  ...
  if (window.__btLazyStartupReady === false) return false;
  return !!(document.getElementById('header-month') && document.getElementById('page-overview'));
}

function attemptEarlyShellReveal() {
  if (!isEarlyShellRevealReady()) return false;
  __btEarlyShellRevealed = true;
  document.documentElement.setAttribute('data-boot-mode', 'early-shell');
...
async function runBootDataHydration(ctx = {}) {
  ...
  try {
    ...
    await yieldStartupLogoFrames(2);
    try { prepareUiAfterDataLoad({ render: false }); } catch (_) {}
    await yieldStartupLogoFrames(2);
    try { renderAll({ visibleOnly: true, deferHeavy: false }); } catch (_) {}
    await yieldStartupLogoFrames(2);
